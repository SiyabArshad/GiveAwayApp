...
function DetailsPageScreen(props) {
  ...
  return (
    <Screen style={styles.screen}>
      ...
      {/* Images*/}
      <Image
        style={{ width: RFPercentage(35), height: RFPercentage(25), borderRadius: RFPercentage(1), marginTop: RFPercentage(8) }}
        source={data?.productimage ? { uri: data?.productimage } : require("../../assets/Images/p1.png")}
      />
      ...
      <ScrollView style={{ flex: 1, width: "100%", marginTop: RFPercentage(4) }}>
        <View style={{ justifyContent: "center", alignItems: "center", width: "100%" }}>
          <View style={{ width: "100%", justifyContent: "center", alignItems: "flex-start", alignSelf: "center" }}>
            <View style={{ marginTop: RFPercentage(1.7), width: "90%", justifyContent: "flex-start", alignItems: "center", flexDirection: "row", alignSelf: "center" }}>
              ...
              <View style={{ flexDirection: "row", justifyContent: "center", alignItems: "center", position: "absolute", right: 0 }}>
                <Text style={{ fontWeight: "bold", fontSize: RFPercentage(2.2), color: Colors.primary }}>{data?.condition ? data?.condition : "Good"}</Text>
              </View>
            </View>
          ...
          <View style={{ marginTop: RFPercentage(3), width: "90%", justifyContent: "center", alignItems: "flex-start", alignSelf: "center" }}>
            <Text style={{ color: Colors.green, fontWeight: "bold", fontSize: RFPercentage(2.2) }}>פרטים</Text>
            <Text style={{ lineHeight: RFPercentage(2.8), marginTop: RFPercentage(2), color: Colors.black, fontSize: RFPercentage(1.8) }}>{`Available at ${data?.location}`}</Text>
          </View>
          ...
          >
            <View style={{ marginLeft: RFPercentage(2), justifyContent: "center", alignItems: "center" }}>
              <Text style={{ color: Colors.white, fontSize: RFPercentage(2), left: RFPercentage(-2) }}>פרטי בעלים:</Text>
            ...
            <TouchableOpacity activeOpacity={0.8} style={{ position: "absolute", right: RFPercentage(2) }}>
              ...
              >
                <MaterialCommunityIcons name="phone" style={{ fontSize: RFPercentage(3.2) }} color={Colors.white} />
              </TouchableOpacity>
            </TouchableOpacity>
          </View>
        </View>
